...
A IA roda só no Diário. Localmente usa Ollama; no Space, transformers + ZeroGPU.
"""
...
# modelo 3B (≤4B = badge Tiny Titan) na GPU grátis do Space (ZeroGPU)
MODELO = "Qwen/Qwen2.5-3B-Instruct"
tokenizer = AutoTokenizer.from_pretrained(MODELO)
model = AutoModelForCausalLM.from_pretrained(
    MODELO, torch_dtype=torch.bfloat16, device_map="auto",
)
...
# ===========================================================================
# Geração do Diário (IA) via transformers, na GPU grátis do Space (ZeroGPU)
# ===========================================================================
...
def gerar_reflexao_stream(pergunta, texto):
    ...
    entradas = tokenizer([texto_prompt], return_tensors="pt").to(model.device)
    streamer = TextIteratorStreamer(
        tokenizer, skip_prompt=True, skip_special_tokens=True,
    )
    kwargs = dict(
        **entradas, streamer=streamer, max_new_tokens=200,
